...
(function () {
  ...
  function updateModalTitle(replyToName) {
    ...
    if (replyToName) {
      ...
      if (ctx) {
        ctx.textContent = "Replying to " + replyToName;
  ...
  function injectModal() {
    ...
    document
      .getElementById("cm-submit")
      .addEventListener("click", submitComment);
    document
      .getElementById("comment-modal-overlay")
      ...
      list.forEach(function (c) {
        if (c.gravatar_hash)
          map[c.id] =
            ...
            "?s=40&d=404";
        var nested =
          c.replies && c.replies.data ? c.replies.data : c._children || [];
  ...
  function renderComment(c, avatarMap) {
    ...
    var html =
      ...
      "</span>" +
      '<button class="btn-reply-comment" data-id="' +
      esc(c.id) +
      '" data-author="' +
      author +
      '">Reply</button>' +
      ...
      "</div>";
    var nested =
      c.replies && c.replies.data && c.replies.data.length
        ? c.replies.data
    ...
    if (nested.length) {
      ...
      nested.forEach(function (r) {
        html += renderComment(r, avatarMap);
      });
      html += "</div>";
    }
    return html + "</div>";
  ...
  function renderComments(container, comments, avatarMap) {
    ...
    container.innerHTML = comments
      .map(function (c) {
        return renderComment(c, avatarMap);
      })
      .join("");
  }
  ...
  function fetchComments(section) {
    ...
    fetchPage(1)
      .then(function () {
        return buildAvatarMap(allComments);
      })
      .then(function (avatarMap) {
        renderComments(list, allComments, avatarMap);
      })
      .catch(function () {
        list.innerHTML =
  ...
  function initCommentSections() {
    var sections = document.querySelectorAll(
      ".comments-section[data-cusdis-app-id]",
    );
