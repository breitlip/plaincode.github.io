...
function usage() {
  console.error(
    "Usage: node tools/import-wp-comments.js <export.xml> <postgres-url>",
  );
  process.exit(1);
}
...
for (const item of items) {
  ...
  let postSlug =
    postSlugRaw || postLink.replace(/\/$/, "").split("/").pop() || "";

  const pageId = SLUG_MAP[postSlug];
  if (!pageId) {
    // Only warn if the post has comments
    const commentBlocks = allMatches(
      content,
      /<wp:comment>([\s\S]*?)<\/wp:comment>/,
    );
    if (commentBlocks.length > 0) {
      console.warn(
        `  ⚠  Skipping "${postSlug}" (${commentBlocks.length} comment(s)) — not in SLUG_MAP. Add it if needed.`,
      );
    }
    continue;
  }
  ...
  for (const cm of comments) {
    ...
    const dateStr =
      getTag(c, "wp:comment_date_gmt") || getTag(c, "wp:comment_date");
...
(async () => {
  ...
  for (const r of toInsert) {
    const parentId =
      r.wpParent && r.wpParent !== "0" ? wpIdToCuid[r.wpParent] || null : null;
    ...
    try {
      ...
      console.log(
        `  ✓  [${r.pageId}] ${r.nickname}: "${r.body.slice(0, 60).replace(/\n/g, " ")}…"`,
      );
      inserted++;
    } catch (err) {
